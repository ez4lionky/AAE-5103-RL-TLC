...
class IntersectionVehicleGenerator:
    ...
    def __init__(self, params, world, I, fns=("vehicle_trajectory", "lane_vehicles", "history_vehicles", "vehicle_distance"),
        ...
        # self.ob_shape, self.map_shape = result_shape[targets[0]], result_shape[targets[1]]
        if len(targets) == 1 and targets[0] == 'vehicle_map':
            self.ob_shape = result_shape[targets[0]]
        else:
            self.ob_shape = [sum(result_shape[k][0] for k in targets)]
    ...
    #                 continue
    #             transform_tuple = vehicle_location_mapper(vehicle_position, area_length,
    #                                                       area_width)  # transform the coordinates to location in grid
