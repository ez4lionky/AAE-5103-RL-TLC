...
import os
import pickle
from agent.dqn import DQNNet, DQNMapNet
from agent.dqn_rnn import TLC_DQN
import utils as u


class DQNAgent(RLAgent):
    model_constructor = {'FC': DQNNet, 'CNN': DQNMapNet, 'Full': TLC_DQN}
    ...
    def __init__(self, action_space, ob_generator, reward_generator, iid, parameters, world, device):
        ...
        map_shape = ob_generator[1].ob_shape
        ...
        self.model = Model(hidden_size, ob_length, map_shape, action_space=8, device=device).to(device)
        self.target_model = Model(hidden_size, ob_length, map_shape, action_space=8, device=device).to(device)
    ...
    def remember(self, ob, obs_map, action, reward, next_ob, next_obs_map, edges_idx, edge_features,
                 next_edge_features):
        data = ob, obs_map, action, reward, next_ob, next_obs_map, edges_idx, edge_features, next_edge_features
        self.memory.append(data)

    def get_action(self, ob, obs_map, edges_idx, edge_features):
        # print("obs_map:", obs_map.shape)  # 1, 3, 2, 12, 60
        ...
        self.model.eval()
        act_values, att_weights = self.model(ob, obs_map, edges_idx, edge_features)
    ...
    def model_update(self, obs, obs_map, actions, rewards, next_obs, next_obs_map, edges_idx, edge_features,
                     next_edge_features, weights=None):
        ...
        state_action_values, _ = self.model(obs, obs_map, edges_idx, edge_features)
        # print("state_action_values:", state_action_values.shape)
        state_action_values = state_action_values.gather(1, actions)

        ## Compute V(s_{t+1}) for all next states.
        # with torch.no_grad():
        next_state_values, _ = self.target_model(next_obs, next_obs_map, edges_idx, next_edge_features)
    ...
    def new_replay(self, data, adjust_epsilon=True):
        obs, obs_map, actions, rewards, next_obs, next_obs_map, edges_idx, \
        edge_features, next_edge_features, tree_idx, weights = data
        self.model.train()
        abs_errors = self.model_update(obs, obs_map, actions, rewards, next_obs, next_obs_map, edges_idx,
                                       edge_features, next_edge_features, weights)
        if self.use_prioritized:
